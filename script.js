...
// ---------------------------------------------------------------------------
const videosDataset = [
  { name: 'Naa Anveshana Marriage And Divorce',                                          views: 7000000,  likes: 154000, comments: 12500, date: '~2023–2024', category: 'culture', country: 'India', titlePattern: 'Personal Update', thumbPattern: 'Emotional/Story' },
  { name: 'Andhra Pradesh Politics | Zimbabwe Hyperinflation Story Explained',           views: 6200000,  likes: 128000, comments: 8400,  date: '~2023–2024', category: 'culture', country: 'Zimbabwe', titlePattern: 'Deep Dive', thumbPattern: 'Shock/Comparison' },
  { name: 'Things To Do In Cairo Egypt | Grand Egyptian Museum',                         views: 6000000,  likes: 122000, comments: 7500,  date: '~2023–2024', category: 'culture', country: 'Egypt', titlePattern: 'Travel Guide', thumbPattern: 'High Production' },
  { name: 'Sleeping In Amazon Rainforest Brazil',                                        views: 5500000,  likes: 114000, comments: 9200,  date: '~2021–2022', category: 'adventure', country: 'Brazil', titlePattern: 'Extreme Experience', thumbPattern: 'Action/POV' },
  { name: 'Type Of Thai Massage In Thailand',                                            views: 5100000,  likes: 98000,  comments: 6800,  date: '~2022–2023', category: 'culture', country: 'Thailand', titlePattern: 'Cultural Experience', thumbPattern: 'Intrigue/Curiosity' },
  { name: 'Madagascar Street Food | Naa Anveshana Food Vlogs',                           views: 4900000,  likes: 88000,  comments: 5400,  date: '~2022–2023', category: 'food', country: 'Madagascar', titlePattern: 'Food Review', thumbPattern: 'Visceral/Sensory' },
  { name: 'China Village Life and Chinese Farmer Lifestyle',                             views: 4800000,  likes: 92000,  comments: 7800,  date: '~2023–2024', category: 'culture', country: 'China', titlePattern: 'Slice of Life', thumbPattern: 'Authentic/Raw' },
  { name: 'Beijing to Shanghai High Speed Bullet Train in China',                       views: 4800000,  likes: 87000,  comments: 6200,  date: '~2023–2024', category: 'transport', country: 'China', titlePattern: 'Transport/Tech', thumbPattern: 'Futuristic' },
  { name: 'Gold Mines in Madagascar',                                                   views: 4700000,  likes: 82000,  comments: 5900,  date: '~2022–2023', category: 'adventure', country: 'Madagascar', titlePattern: 'Documentary', thumbPattern: 'Gritty/Real' },
  { name: 'Afghanistan to Pakistan Traveling By Car | Torkham Border',                  views: 4600000,  likes: 112000, comments: 14500, date: '~2022–2023', category: 'adventure', country: 'Af/Pak', titlePattern: 'High Stakes Border', thumbPattern: 'Danger/Tension' },
  { name: 'Naa Anveshana Arctic Cruise Ship Journey',                                   views: 4400000,  likes: 76000,  comments: 4800,  date: '~2024–2025', category: 'luxury', country: 'Arctic', titlePattern: 'Luxury/Exclusive', thumbPattern: 'Aspirational' },
  { name: 'Fiji to Australia to Hong Kong to China Wonderful Journey',                  views: 4400000,  likes: 80000,  comments: 5100,  date: '~2022–2023', category: 'adventure', country: 'Multi-Country', titlePattern: 'Epic Journey', thumbPattern: 'Montage/Map' },
  { name: 'Tonga Capital City And Local Market',                                        views: 4300000,  likes: 72000,  comments: 4200,  date: '~2022–2023', category: 'culture', country: 'Tonga', titlePattern: 'Local Tour', thumbPattern: 'Vibrant/Colors' },
  { name: 'Naa Anveshana Problems In China 🇨🇳',                                        views: 4200000,  likes: 97000,  comments: 11200, date: '~2023–2024', category: 'adventure', country: 'China', titlePattern: 'Challenge/Conflict', thumbPattern: 'Emotional/Red Arrow' },
  { name: 'The Three Gorges Dam On The Yangtze River China',                            views: 3800000,  likes: 67000,  comments: 4500,  date: '~2023–2024', category: 'culture', country: 'China', titlePattern: 'Mega-structure', thumbPattern: 'Scale/Magnitude' }
];
...
function initVectorMap() {
  ...
  window.addEventListener('resize', () => {
    if (vectorMapInstance && typeof vectorMapInstance.updateSize === 'function') {
...
function renderPerformanceTable() {
  ...
  if (filtered.length === 0) {
    tableBody.innerHTML = `<tr><td colspan="9" style="text-align: center; padding: 2rem; color: var(--text-muted);">No records found matching filters.</td></tr>`;
    return;
  }
  ...
  filtered.forEach(v => {
    ...
    html += `
      <tr>
        ...
        <td>${v.country || '-'}</td>
        <td><span style="font-size:0.85rem; padding:2px 6px; background:rgba(255,255,255,0.05); border-radius:4px; white-space:nowrap;">${v.titlePattern || '-'}</span></td>
        <td><span style="font-size:0.85rem; padding:2px 6px; background:rgba(255,255,255,0.05); border-radius:4px; white-space:nowrap;">${v.thumbPattern || '-'}</span></td>
      </tr>
    `;
  });
...
function setupPresentationMode() {
  ...
  btn.addEventListener('click', () => {
    ...
    if (isPres) {
      btn.innerHTML = '<i class="fa-solid fa-person-chalkboard"></i><span>Exit Presentation</span>';
    } else {
      btn.innerHTML = '<i class="fa-solid fa-person-chalkboard"></i><span>Executive Presentation Mode</span>';
    }
...
function setupSidebarEvents() {
  ...
  // Close sidebar on link clicks on mobile viewports with header offset compensation
...
// 15. EXECUTIVE BOARDROOM BRIEFING RENDERER
function renderExecutiveSummary() {
  ...
  const opportunitiesList = document.getElementById('exec-opportunities-list');
  const recommendationsList = document.getElementById('exec-recommendations-list');

  if (!findingsList || !risksList || !opportunitiesList || !recommendationsList) return;

  const findings = [
    "Highest documented growth in APAC region travel channels.",
    "Subscriber velocity is accelerating month-over-month.",
    "Cross-continent travel generates the highest engagement.",
    "African destinations are currently outperforming Asian ones.",
    "Audience retention in long-form content is stable."
  ];

  const risks = [
    "Heavy reliance on YouTube ad revenue (undiversified).",
    "Production costs scaling faster than revenue.",
    "Burnout risk due to aggressive 12-14/mo upload schedule.",
    "Platform algorithm changes impacting suggested video traffic.",
    "Geopolitical instability in key travel corridors."
  ];

  const opportunities = [
    "Untapped markets in East Asia (Japan, South Korea).",
    "Premium sponsored integrations with travel tech brands.",
    "Exclusive community memberships (Patreon/YT Memberships).",
    "Curated group travel packages hosted by creator.",
    "Merchandising targeted at the adventure travel niche."
  ];

  const recommendations = [
    "Diversify revenue streams immediately.",
    "Establish standard operating procedures for production.",
    "Explore strategic brand partnerships for Q3/Q4.",
    "Launch tiered membership program for core audience.",
    "Pilot a small-scale group travel package to test conversion."
  ];
  ...
  const populateList = (element, data) => {
    ...
    data.forEach((item, idx) => {
      html += `
        <li class="exec-item">
          <span class="exec-item-num">0${idx + 1}.</span>
          <span>${item}</span>
        </li>
      `;
    });
  ...
  populateList(findingsList, findings);
  populateList(risksList, risks);
  populateList(opportunitiesList, opportunities);
  populateList(recommendationsList, recommendations);
}

// 16. UNDERPERFORMING CONTENT RENDERER
function renderUnderperformingContent() {
  const tableBody = document.getElementById('underperforming-table-body');
  if (!tableBody) return;

  const weakVideos = [
    { title: "Long Flight to South America...", views: "385K", likes: "12K", deficit: "Transit Trap (No Hook)" },
    { title: "Europe Trip Part 4", views: "410K", likes: "15K", deficit: "Context Deficit (Sequential)" },
    { title: "A nice day in the city of Paris", views: "450K", likes: "18K", deficit: "Vague Descriptor" },
    { title: "Walking around the streets in the morning", views: "290K", likes: "9K", deficit: "Unstructured Format" }
  ];

  let html = '';
  weakVideos.forEach(vid => {
    html += `
      <tr>
        <td style="font-weight: 600;">${vid.title}</td>
        <td style="font-family: var(--font-mono); color: var(--text-secondary);">${vid.views}</td>
        <td style="font-family: var(--font-mono); color: var(--text-secondary);">${vid.likes}</td>
        <td><span class="badge-tag" style="background: rgba(255, 56, 56, 0.1); color: var(--color-red); border-color: rgba(255, 56, 56, 0.2);">${vid.deficit}</span></td>
      </tr>
    `;
  });
